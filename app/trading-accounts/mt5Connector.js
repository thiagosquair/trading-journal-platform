...
export class MT5Connector {
  ...
  async connectToAccount(accountId, login, password, server) {
    try {
      console.log("Connecting to account via backend:", { accountId, login, password, server })
  ...
  async disconnectFromAccount(accountId) {
    try {
      console.log("Disconnecting from account via backend:", { accountId })
  ...
  async getAccountInfo(accountId) {
    try {
      console.log("Getting account info from backend:", { accountId })
  ...
  async getHistory(accountId, startDate, endDate) {
    try {
      console.log("Getting history from backend:", { accountId, startDate, endDate })
      const response = await fetch(
        `${BACKEND_URL}/api/mt5/history?accountId=${accountId}&startDate=${startDate}&endDate=${endDate}`,
      )
      const data = await response.json()
