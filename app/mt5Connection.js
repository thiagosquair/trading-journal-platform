import { mt5Connector } from "./trading-accounts/mt5Connector"

class MT5Connection {
  constructor() {
    this.api = mt5Connector
    this.connections = {}
  }
  ...
  async getTradeHistory(accountId, startDate, endDate) {
    try {
      ...
      // Get history orders
      const historyOrders = await conn.connection.getHistoryOrdersByTimeRange(new Date(startDate), new Date(endDate))

      // Get history deals
      const historyDeals = await conn.connection.getDealsByTimeRange(new Date(startDate), new Date(endDate))
...
export default new MT5Connection()
